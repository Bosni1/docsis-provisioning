...
class Entry:
    ...
    class Combo(Field):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.entry.config (width = self.formeditor.entrywidth )
            
            for cv, cd in self.field.choices:
                self.widget.insert ( END, cv )
    
    class Radio(Field):
        def __init__(self, *args, **kwargs):
            Entry.Field.__init__(self, *args, **kwargs)
            self.widget = Tix.Frame(self.parent)
            self.idx_map = {}
            for idx, (cv, cd) in enumerate(self.field.choices):
                b = Tix.Radiobutton ( self.widget, value=cv, text=cd, 
                                      variable=self.variable, anchor=W,
                                      selectcolor="yellow")
                b.pack()            
    ...
    class Select(Field):
        def __init__(self, *args, **kwargs):
            ...
            self.widget = Tix.Select ( self.parent, radio=True,allowzero=True,                                       
                                       orientation=VERTICAL, command=self.selection_change)
            self.idx_map = {}
            self.val_map = {}
            for idx, (cv, cd) in enumerate(self.field.choices):
                self.idx_map[cv] = idx
        ...
        def selection_change(self, *args):
            ...
            if state == '0': return
                ...
                try:
                    ...
                    print "value=", self.val_map[int(idx)]
                    self.variable.set ( self.val_map[int(idx)] )
                finally:
    ...
    class StaticReference(Reference):    
        def __init__(self, *args, **kwargs):
            Entry.Reference.__init__(self, *args, **kwargs)        
    ...
    class ComboReference(Reference, eventemitter):
    ...
    class SearchReference(ComboReference):
        def __init__(self, *args, **kwargs):
            eventemitter.__init__ ( self, [ "navigate", "current_record_changed" ] )
            Entry.Reference.__init__(self, *args, **kwargs)                        
    ...
    class ArrayText(Array, ArrayEntryTextMixin, ArrayEntryButtonMixin):
        pass
    
    
    class ArrayStatic(Array, ArrayEntryLabelMixin):
        pass
